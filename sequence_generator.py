...
class SequenceGenerator(object):
    ...
    def generate_sequences(self, n_sequences):
        ...
        for i in range(n_sequences):
            ...
            time_noise = np.random.normal(loc=0, scale=self.time_sample_noise, size=time.size)
            time_noise = time_noise * ((time_noise + time) < last_sample_time)
            time_noise = time_noise * ((time_noise + time) > self.time_span[0])
            time += time_noise
            time = np.sort(time)
    ...
    def generate_dataset(self, **kwargs):
        ...
        for set_i, name in enumerate(sets_names):
            ...
            sequences, real_values, times, params, noise, dense_time, lengths = self.generate_sequences(
                datasets[name]["n_sequences"])
    ...
    def plot_n_examples(self, subset_name="training", n_examples=3, data_path=None):
        ...
        for i in idx:
            ...
            plt.errorbar(subset["time"][i], subset["sequences"][i], yerr=subset["noise"][i], fmt="o", ms=5,
                         label="samples")
            plt.xlabel("time")
